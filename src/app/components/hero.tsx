    ...
    >
      <div className='w-3/5 h-80 flex flex-col space-y-5 ml-20'>
        <section className='flex justify-start items-center space-x-2'>
          <p className='bg-[#F92424] w-10 h-10 flex justify-center items-center font-bold text-2xl text-white'>0{data.id + 1}</p>
          <h1 className='font-base text-white uppercase'>{data?.label}</h1>
        </section>
        <h2 className='text-[80px] font-bold text-white leading-[95%] uppercase'>{data?.title}</h2>
...
const HeroSlider = () => {
  ...
  return (
    ...
    >
      <div className='w-screen h-1/2 flex items-center bg-black bg-opacity-5 bg-blur bg-gradient-to-t from-gray-900'>
        {/* TEXT SECTION */}
        <TextSection data={currentData} />
        {/* SLIDER SECTION */}
        <div className='w-2/3 h-auto -mr-32'>
